...
author: "Anna Magoline & Steven Mayher"
date: "`r Sys.Date()`"
output:
...
I've selected five variables (v044, v145, v021, v023 and v139) which I'll describe shortly. You will make your own choices, of course, and you'll need to provide more information on each variable in a codebook.

To help you think about the chunk of code below, note that the code below does the following things:

1. Filter the dat===a to the actual counties that are ranked in the Rankings (this eliminates state and USA totals, mainly.)
...
chr_2021 <- chr_2021_raw %>%
    ...
    mutate(some_college = 100*some_college,
           home_ownership = 100*home_ownership,
...
First, I'm going to make a binary categorical variable using the `severe_housing_cost_burden` variable. Note that categorizing a quantitative variable like this is (in practice) a terrible idea, but we're doing it here so that you can demonstrate some facility with modeling using a categorical variable.
...
Let's check - we should still have 31 rows, but now we should have 9 columns (variables), since we've added the `severe_housing_cost_burden_cat` column to the data.
...
Now, I'm going to demonstrate the creation of a multi-category variable based on the `broadband_access` variable. I'll briefly reiterate that categorizing a quantitative variable like this is (in practice) a terrible, no good, very bad idea, but we're doing it anyway for pedagogical rather than scientific reasons.
...
mosaic::favstats(broadband_access ~ temp3, data = chr_2021) %>% 
    kable(digits = 3)
```
...
So, I've created two multi-categorical variables, but I will just retain the 3-category version (which I called `broadband_access_cat`) and drop the other temporary efforts.
...
- I've included only 10 variables:
    ...
    - my two categorical variables `severe_housing_cost_burden_cat` and `broadband_access_cat` which should each be factors <fct> with appropriate levels specified, followed by numerical codes
...
broadband_access | (v021) High School Graduation Rate
severe_housing_cost_burden | (v023) severe_housing_cost_burden Rate
food_insecurity | (v139) Food Insecurity Rate
severe_housing_cost_burden_cat | 2 levels: low = severe_housing_cost_burden below 3.9%, or high
broadband_access_cat | 3 levels: bottom = broadband_access below 88%, middle or top = 90.9% or above
...
If I did have some missingness, then I would probably want to summarize this by state, so that I could compare the results. Here's a way to look at this just for the `income_inequality` variable.

```{r, message = FALSE}
mosaic::favstats(broadband_access ~ state, data = chr_2021) %>%
...
```{r}
saveRDS(chr_2021, file = "chr_2021_Anna_Magoline_and_Steven_Mayher.Rds")
```
